...
| | |
|---|---|
| `deepsteve start` | deploy, register the service, start the daemon (also the upgrade step after `npm install -g deepsteve@latest`) |
| `deepsteve stop` | stop the daemon — sessions live in tmux and survive |
| `deepsteve restart` | restart, after confirming in the browser (`--force` moves the confirmation to your terminal) |
| `deepsteve status` | service state, port, health, log locations — read-only |
| `deepsteve uninstall` | remove the service, `~/.deepsteve`, and deepsteve's own tmux server |
