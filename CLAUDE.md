...
- **`read_session_screen` MCP tool**: ungated read-only companion to `meta_type` — returns the last N lines (default 40, max 200) of a session's ANSI-stripped scrollback tail plus `state` (`idle`/`busy`/`unknown`) and `seconds_since_output`. TUI agents redraw frames constantly, so the tail *approximates* the current screen. Use it to check what a session is doing or to verify typed input landed — no more `browser_eval` poking at `window.__deepsteve` internals.
