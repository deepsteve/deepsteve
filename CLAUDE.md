...
- **Host allowlist** — rejects any request/WS upgrade whose `Host` hostname isn't `localhost`/`127.0.0.1`/`[::1]` (or a `--allow-host` you add). This is what actually stops DNS rebinding (the rebind domain shows up in `Host`).
