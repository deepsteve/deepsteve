...
```bash
./restart.sh --force --refresh             # step 1: prints the live blast radius + the exact confirm command. No restart.
./restart.sh --force --prompt "Restarting - 3 sessions running under tmux, none interrupted" --refresh   # step 2: restarts after re-checking the text
```
...
The server owns the wording (`GET /api/restart-prompt`, derived from the live sessions *and their engines* — a tmux-backed agent is detached, not killed, so the line says how many actually get interrupted); step 2 re-validates the echoed `--prompt` text against the server and aborts if it's stale or forged, so the number you approve is always the real one. The forced path still does a full graceful deploy + restart (it just skips the `/api/request-restart` POST). **Do not allowlist `./restart.sh` (especially the `--force --prompt` form)** — the guarantee that a restart can never happen unilaterally (e.g. by an agent) rests entirely on that command staying behind Claude Code's permission prompt.
